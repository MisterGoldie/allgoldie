...
export const app = new Frog({
  ...
  title: '$GOLDIES and NFT Checker on Polygon',
}).use(
  neynar({
...
async function getGoldiesBalance(address: string): Promise<string> {
  try {
    ...
    const contract = new ethers.Contract(GOLDIES_TOKEN_ADDRESS, GOLDIES_ABI, provider);
...
async function getNFTBalance(address: string): Promise<string> {
  try {
    ...
    const balance = await contract.balanceOf(address);
    console.log('Fetched NFT balance:', balance.toString());
    return balance.toString();
  } catch (error) {
    console.error('Error in getNFTBalance:', error);
    return 'Error: Unable to fetch NFT balance';
  }
}
    ...
    image: (
      <div style={{
        ...
        <p style={{
          ...
          color: 'white',
          textShadow: '2px 2px 4px rgba(0,0,0,0.5)',
        }}>Choose what to check</p>
      </div>
    ),
    intents: [
      <Button action="/check-goldies">Check $GOLDIES</Button>,
      <Button action="/check-nft">Check NFT</Button>,
    ],
  });
});

app.frame('/check-goldies', async (c) => {
...
app.frame('/check-nft', async (c) => {
  ...
  let nftBalanceDisplay = "Unable to fetch NFT balance";
  let errorDetails = "";
  ...
  try {
    ...
    console.log('Fetched NFT balance:', nftBalance);

    nftBalanceDisplay = nftBalance === '0' 
      ? "You don't own any of these NFTs on Polygon"
      : `You own ${nftBalance} NFT(s) from this collection`;

    console.log('Final NFT balance display:', nftBalanceDisplay);
  } catch (error) {
    console.error('Detailed error in NFT balance check:', error);
    if (error instanceof Error) {
      console.error('Error name:', error.name);
      console.error('Error message:', error.message);
      console.error('Error stack:', error.stack);
    }
    nftBalanceDisplay ="Error fetching NFT balance";
    errorDetails = error instanceof Error ? `${error.name}: ${error.message}` : 'Unknown error';
  }
  ...
  return c.res({
    image: (
      <div style={{ 
        ...
        <h1 style={{ fontSize: '60px', marginBottom: '20px', textAlign: 'center' }}>Your NFT Balance</h1>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
          {pfpUrl ? (
            <img 
              src={pfpUrl} 
        ...
        <p style={{ fontSize: '52px', textAlign: 'center' }}>{nftBalanceDisplay}</p>
        {errorDetails && <p style={{ fontSize: '26px', color: 'red', marginTop: '10px', textAlign: 'center' }}>Error: {errorDetails}</p>}
      </div>
    ),
    intents: [
      <Button action="/">Back to Menu</Button>,
      <Button.Link href={`https://polygonscan.com/token/${NFT_CONTRACT_ADDRESS}`}>View on Polygonscan</Button.Link>,
      <Button action="/check-nft">Refresh</Button>,
    ],
  });
});
